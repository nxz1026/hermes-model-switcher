...
运行 Python 脚本测试所有 provider（见 `scripts/verify_model_switcher.py`）。

### 3. 切换模型

#### 方式 A: `hermes-switch` CLI（推荐，v0.1.0+）

```bash
# 列出所有可用模型
hermes-switch --list

# 切换到指定模型（provider/model_id 格式）
hermes-switch minimax-cn/MiniMax-M3
hermes-switch deepseek/deepseek-chat
hermes-switch openrouter/claude-sonnet-4
hermes-switch custom/xopqwen36v35b  # 切回讯飞
```

切换后 config.yaml 自动更新，gateway 需手动 reload。

#### 方式 B: `hermes config set`（手动逐字段）

```bash
hermes config set model.default "deepseek-chat"
hermes config set model.api_key "你的密钥"
...
### 从 GitHub 同步到本地 skill

```bash
cd ~/.hermes/skills/devops/hermes-model-switcher
...
```bash
git init
git remote add origin https://github.com/nxz1026/hermes-model-switcher.git
git fetch origin
```
...
```bash
git clone --depth 1 https://github.com/nxz1026/hermes-model-switcher.git /tmp/hms
cd /tmp/hms
# 修 pyproject.toml: 添加 [tool.setuptools.packages.find] include = ["hermes_switch*"]
pip install --break-system-packages -e .
```

**必改 pyproject.toml** — 仓库默认无 `[tool.setuptools.packages.find]`，pip install 会报 `Multiple top-level packages discovered` 错误。添加：
```toml
[tool.setuptools.packages.find]
include = ["hermes_switch*"]
```
...
| 版本 | 变更 |
|------|------|
| 0.1.0 | 当前：从 `minimax-cn/MiniMax-M3` 到 `minimax-cn/MiniMax-M3`，config 自动写 `provider: minimax-cn` |
...
| minimax-cn | `api.minimaxi.com/anthropic` | anthropic | ❌ 404 endpoint（验证失败，base_url 含 /anthropic 但 API 返回 404） |
...
- **⚠️ 2026-06-13 新发现**: `hermes-switch` CLI 维护**独立**的 provider 别名表（如 `xunfei/xopqwen36v35b`），与 Hermes **核心 config 的 provider 字段命名不一致**。CLI 列出的 `xunfei/...` 不等于 Hermes core 认的 `custom` provider。CLI 能列但不能直接切到核心——CLI 报 `xunfei/MiniMax-M3` 可用，Hermes 核心会报 `Unknown provider`。**直接编辑 ~/.hermes/config.yaml + `kill -USR1 <pid>` reload gateway 是唯一可靠路径**。CLI 仅用于查看和临时测试。
- **Custom provider auto-discovery**: when users add providers via `hermes model`, update/verify the script so it reads `custom_providers[]` dynamically instead of hard-coding every endpoint. See `references/custom-provider-auto-discovery.md`.
- **⚠️ 2026-06-13 新发现**: `/usr/local/bin/hermes-switch` 可能存在但包缺失，表现为 `ModuleNotFoundError: No module named 'hermes_switch'`。判断 CLI 可用不能只看 `command -v hermes-switch`，必须实际跑 `hermes-switch --list`。修复路径：重新从 `https://github.com/nxz1026/hermes-model-switcher.git` 安装，并确认 pyproject.toml 含 `[tool.setuptools.packages.find] include = ["hermes_switch*"]`。